  ...
  >
    ...
    <template v-if="!shouldBeCollapsed">
      <div class="flex">
        ...
        <div class="w-full flex items-center" :class="{ 'mb-3': !viaResource }">
      ...
      <div class="flex mb-3 nova-select-search">
          <div v-for="search in searches" class="form-check">
            <input type="checkbox"
                  class="checkbox mr-1 form-check-input"
                  name="search_options"
                  :id="'search_'+search"
                  :value="search"
                  v-model="selectedSearches"
                  @change="changeSearch()">
            <label class="select-none capitalize mr-4 form-check-label"
                  :class="'search-'+search"
                  :for="'search_'+search">{{search.replace(/_/g," ")}}</label>
          </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getSearches() {
      ...
      return Nova.request()
        .get('/nova-api/' + this.resourceName + '/get/searches/')
        .then(response => {
  ...
  computed: {
    ...
    resourceRequestQueryString() {
      return {
        ...
        relationshipType: this.relationshipType,
        searchAttributes: this.selectedSearches
      }
    },
